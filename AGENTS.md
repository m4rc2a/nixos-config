...
- **Forgejo (roo6)**: Replaced GitLab (2026-08). Serves `git.roo6.lan` via nginx → `127.0.0.1:3000`, sqlite3 DB, Forgejo's own SSH on port **2222** (not 22). `SECRET_KEY` comes from sops secret `forgejo_session_key`.
