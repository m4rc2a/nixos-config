...
- **Caddy subdomains (roo6)**: All web services sit behind Caddy `80/443` by hostname: `git.zander.cloud`→Forgejo:3000, `hass.zander.cloud`→HA:8123, `jellyfin.zander.cloud`→8096, `invidious.zander.cloud`→3001 (moved off 3000 to avoid clash with Forgejo), `radarr.zander.cloud`→7878. Forgejo `DOMAIN`/`ROOT_URL` = `git.zander.cloud` (git clone over HTTPS via Caddy; port 2222 SSH not reverse-forwarded).
- **Reverse-web access via shelog**: `modules/services/ssh-reverse-tunnel.nix` now supports `extraReverseForwards`; roo6 forwards `-R 2443:localhost:22` **and** `-R 8443:localhost:443` (Caddy). Clients run `systemd.roo6-web-tunnel` (`-L 127.0.0.1:443:127.0.0.1:8443 -p 20022 -l marc shel:org`, needs `CAP_NET_BIND_SERVICE` to bind 443 non-root) + `networking.hosts` maps `*.zander.cloud`→`127.0.0.1`. Then `https://<svc>.zander.cloud` just works from the client.
- **TLS via Caddy internal CA (roo6)**: No public DNS API needed (IONOS developer API is not available for domain-only contracts). Each virtualHost in `modules/services/caddy.nix` sets `tls internal` — Caddy signs `*.zander.cloud` with its own CA. The root CA is stable (created Mar 2026, valid to 2036) at `/var/lib/caddy/.local/share/caddy/pki/authorities/local/root.crt` on roo6 and is committed as `modules/certs/caddy-roo6-root.crt`. Clients trust it via `security.pki.certificateFiles` (wired in `modules/services/roo6-web-tunnel.nix`). If `/var/lib/caddy` is wiped, the CA regenerates → re-extract + re-deploy clients. Certs are locally-trusted only (fine: all access is tunnel-only).
- **Name resolution is hosts-based, NOT public DNS**: `*.zander.cloud` has no public A record (no public endpoint). `networking.hosts` maps the apex + subdomains to `127.0.0.1` on roo6 itself and on every client node (laptop/desktop). Any new device needs hosts + tunnel + CA.
- **NEVER deploy roo6 over its own reverse tunnel**: activation restarts `ssh-reverse-tunnel.service`, killing the tunnel you're deploying through → connection drops mid-deploy, deploy-rs falsely reports "rolled back" (system usually still switches). The new system reached the box fine, but the false-failure is scary. Prefer deploying roo6 from a machine with LAN access (`ssh root@192.168.178.190`) or accept the transient drop. A future fix: `systemd.services.ssh-reverse-tunnel.restartIfChanged = false;`.
